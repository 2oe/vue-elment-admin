...
app.post("/sendSMS", bodyParser.json(), async (req, res) => {
  ...
  try {
    ...
    // 把code存入数据库中
    connection.query(
      `insert into phone_code (phone, code, create_time) values(${
        req.body.phone
      }, ${code}, ${+new Date()})`,
        ...
        connection.query(
          ...
          function(error, results, fields) {
            if (!results[0] || !results[0].id) {
              ...
              // 生成登陆态存到数据库中，后续验证要使用
              let token = `u${results[0].id}_${md5(
                +new Date() + "hello world"
              )}`.slice(0, 16);
              connection.query(
                `insert into token (token, uid, create_time) values("${token}", "${
                  results[0].id
                }", ${+new Date()})`,
                function(error, result, fields) {
                  if (result.insertId) {
                    // 生成登陆态之后获取用户的权限
                    connection.query(
                      `select access.accessname from user,roler,access,user_roler,roler_access where user.id=user_roler.uid and
                            user_roler.rid = roler.id and roler.id = roler_access.rid and roler_access.aid = access.id and user.id = ${
                              results[0].id
                            } group by access.accessname`,
                      function(error, results, fields) {
...
app.post("/updateUser", bodyParser.json(), (req, res) => {
  ...
  connection.query(
    `update user set username="${req.body.username}",phone = ${
      req.body.phone
    }, email = "${req.body.email}", status = ${req.body.status} where id=${
      req.body.id
    }`,
    function(error, results, fields) {
      //   console.log('results...', results);
...
app.post("/deleteUser", bodyParser.json(), (req, res) => {
  ...
  connection.query(`update user set status=0 where id=${req.body.id}`, function(
    error,
    results,
    fields
  ) {
...
app.post("/create", bodyParser.json(), (req, res) => {
  // console.log('req...',req.body);
  connection.query(
    `insert into user(id,username,phone,status,email,password,create_time) values(${
      req.body.id
    },"${req.body.username}",${req.body.phone},${req.body.status},"${
      req.body.email
    }","${req.body.password}",${req.body.timestamp})`,
    function(error, results, fields) {
      if (results.affectedRows) {
        res.json({
          code: 1,
...
app.post(
  "/upload",
  multipart({ autoFiles: true, uploadDir: "server/upload/" }),
  (req, res) => {
    res.json({});
    // 获取表单
    console.log("req...", req.body);
    ...
    for (let i in files) {
      ...
      types.forEach(item => {
        ...
        //   console.log("name...", name[name.length - 1]);
          path.push(
            `http://169.254.78.111:9527/server/upload/${name[name.length - 1]}`
          );
